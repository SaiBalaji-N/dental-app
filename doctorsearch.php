...
try {
    if ($searchType === 'id') {
        $stmt = $conn->prepare("SELECT patientid, name, email, contactno, age, gender FROM patientsignup WHERE patientid LIKE :searchValue");
        $searchPattern = '%' . $searchValue . '%';
        $stmt->bindParam(':searchValue', $searchPattern, PDO::PARAM_STR);
    } else if ($searchType === 'name') {
        $searchPattern = '%' . strtolower($searchValue) . '%';
        $stmt = $conn->prepare("SELECT patientid, name, email, contactno, age, gender FROM patientsignup WHERE LOWER(name) LIKE :searchValue");
        $stmt->bindParam(':searchValue', $searchPattern, PDO::PARAM_STR);
    } else {
